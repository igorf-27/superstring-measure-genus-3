...
print(" * We pick at random a 3x3 period matrix $\\tau$ and compute $\\Xi^{(3)}(\\tau)$")
...
print(" * We check that formula (1.1) for $\\Xi^{(1)}$ for from our paper")
...
print(" * Now we likewise compare formula (1.2) for $\\Xi^{(2)}$ from our paper")
...
print(" * we check that $\\Xi^{(3)}(\\tau_3) = \\Xi^{(1)}(\\tau_1) \\Xi^{(2)}(\\tau_2)$.")
print(" * We calculate $\\Xi^{(3)}$ via Grushevsky's basis.")
